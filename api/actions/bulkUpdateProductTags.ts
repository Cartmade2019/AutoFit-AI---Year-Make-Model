...
export const run: ActionRun = async ({ params, logger, api, connections }) => {
  ...
  let tempShopId: any;
   if (params.shopId) {
      tempShopId = params.shopId
    } else {
       tempShopId = connections.shopify.currentShopId
    }

  const rawShopId = tempShopId;
  ...
  logger.info(`Adding tags to ${totalProducts} products: ${tags.join(", ")}`);

  const BATCH_SIZE = 10;

  // Admin GraphQL: add tags without overwriting
  const mutation = `
    ...
    for (let idx = 0; idx < results.length; idx++) {
      ...
      if (r.status === "fulfilled") {
        ...
        if (userErrors.length > 0) {
          ...
          successCount++;
          logger.info(`Queued tagsAdd for product ${pid}`);
        }
      } else {
        errorCount++;
  ...
  logger.info(`Tags add complete. Total: ${totalProducts}, Success: ${successCount}, Errors: ${errorCount}`);
  return summary;
};
